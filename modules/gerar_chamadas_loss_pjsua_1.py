...
# Configuração de rede para perda de pacotes
print("🌐 Aplicado 5% de perda de pacotes (packet loss).")
subprocess.run("tc qdisc add dev wlp2s0 root netem loss 5%", shell=True)
...
with open(csv_file, mode='w', newline='') as file:
    ...
    for i in range(1, total_chamadas + 1):
        ...
        try:
            ...
            status = "Sucesso"
            print(f"🟢 Chamada {i} encerrada normalmente. Duração: {duracao} segundos. Status: {status}")
        except subprocess.TimeoutExpired:
            fim = time.time()
            duracao = round(fim - inicio, 2)
            processo.send_signal(signal.SIGINT)  # Envia sinal para encerrar o processo

            # ⚡ Corrige a sinalização:
            if 9.5 <= duracao <= 10.5:
                status = "Sucesso"
                print(f"🟢 Chamada {i} atingiu 10s, encerrando normalmente. Duração: {duracao} segundos. Status: {status}")
            else:
                status = "Timeout"
